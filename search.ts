...
export function oldsearch(query: string) {
  return new Promise((res, rej) => {
    let opts = Object.assign({}, DEFAULT_OPTS);

    if (!query) {
      return rej("No query given.");
    }
...
export function parseResponse(responseText, callback) {
  // var _time = Date.now();
...
export function parseDuration(timestampText) {
